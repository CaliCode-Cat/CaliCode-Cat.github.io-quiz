...
//(function () {
  ...
  langBtns.forEach((it, key) => {
    it.addEventListener('click', (evt) => {
      ...
      const shuffledData = window.utils.shuffleArray(dataList[dataId]);
      qns = window.initQuestions(shuffledData);//(ru);//(dataList.dataId);//(dataList[evt.currentTarget.id]);//(dataToFeedRu);
